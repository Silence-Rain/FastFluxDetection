...
class DetailModel(object):
	...
	# 			"?IpSets=%s:32&TableName=%s&Mode=1" % (ip_str, cur_date), proxies=proxy, timeout=10)
	# 		res = r.json()[ip_str+":32"]
	# 		for i in res:
	# 			# 获得对端IP地理位置
	# 			for item in i[1:]:
	# 				if ip not in ret:
	# 					ret[ip] = [item.split("$")[-2].split("-")]
	# 				else:
	# 					ret[ip].append(item.split("$")[-2].split("-"))
	...
	def get_ip_lnglat(self, ips):
		...
		for ip in ips[:100]:
			...
			for i in res:
				...
				ret["self"] = [float(target_pos[0]), float(target_pos[1])]
				# 获得对端IP经纬度
				for item in i[1:]:
					temp = item.split(" ")[-2:]
					opposite_pos.append([float(temp[0]), float(temp[1])])
...
if __name__ == '__main__':
	dm = DetailModel()
	with open("../../data/prod/test.fast-flux") as f:
		for line in f.readlines()[:1]:
			ips = line.split(",")[1:-1]
			print(ips)
			print(dm.get_ip_lnglat(ips))
